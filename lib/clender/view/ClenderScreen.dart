...
class _CalendarpageState extends ConsumerState<Calendarpage> {
  ...
  void initState() {
   ...
   if( _selectedDate == DateTime.now()){
       ...
       return  await Future.delayed(Duration(milliseconds: 300) , () =>    ref.read(MainTasksProvider.future));
     }
  ...
  _listEmployeeTask(BuildContext context){
    ...
    return   RefreshIndicator(
        ...
        onRefresh: ()  async{
          ref.refresh(MainTasksProvider);
      return Future.delayed(Duration(milliseconds: 300) , () =>   ref.read(MainTasksProvider.future));
    },
                           ...
                           onTap:(){
                             Navigator.push(context,
                                 MaterialPageRoute(builder: (context)=>SubTasksScreen(
                                     project_id:int.parse('${data.data[index].projectId}'), main_task_id: data.data[index].id,)));
                             print("idddddd${data.data[index].id}");
                             print("idddddd${data.data[index].projectId}");
                           },
                           child: Container(
                             // width: 90,
                               margin: EdgeInsets.only(
